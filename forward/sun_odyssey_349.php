...
$sql = "SELECT * FROM boots_typen where boots_typ_id = 1";
...
if (isset($_POST['buchungsenden'])) {
  ...
  $sqlPreisProWocheHS = "SELECT preis_pro_woche_hs FROM boots_typen WHERE boots_typ_id = 1";
  $sqlPreisProTagHS = "SELECT preis_pro_tag_hs FROM boots_typen WHERE boots_typ_id = 1";
  $sqlPreisProWocheNS = "SELECT preis_pro_woche_ns FROM boots_typen WHERE boots_typ_id = 1";
  $sqlPreisProTagNS = "SELECT preis_pro_tag_ns FROM boots_typen WHERE boots_typ_id = 1";
  $sqlKaution = "SELECT kaution FROM boots_typen WHERE boots_typ_id = 1";
  ...
  if (
    ...
    $schranke = false;
    $db = mysqli_connect('db5000232553.hosting-data.io', 'dbu387591', 'brun#ITPM8', 'dbs227125');
    $sqlAbfrageBelegt = "SELECT id_1 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
    foreach ($db->query($sqlAbfrageBelegt) as $row) {
      if ($row['id_1'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
        $boots_id = 2;
        break;
      } else {
        $boots_id = 1;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_2 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_2'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = 3;
          break;
        } else {
          $boots_id = 2;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_3 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_3'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = 4;
          break;
        } else {
          $boots_id = 3;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_4 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_4'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = '';
          break;
        } else {
          $boots_id = 4;
    ...
    switch ($boots_id) {
      case 1:
        $tabelle_boots_id = "id_1";
        break;
      case 2:
        $tabelle_boots_id = "id_2";
        break;
      case 3:
        $tabelle_boots_id = "id_3";
        break;
      case 4:
        $tabelle_boots_id = "id_4";
        break;
    }
        ...
        <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
          ...
          <div class="carousel-inner">
            <div class="carousel-item active">
              <img class="d-block w-100" src="../img/yachten/sun_odyssey_349_1.png" alt="First slide">
            </div>
            <div class="carousel-item">
              <img class="d-block w-100" src="../img/yachten/sun_odyssey_349_2.png" alt="Second slide">
            </div>
            <div class="carousel-item">
              <img class="d-block w-100" src="../img/yachten/sun_odyssey_349_3.png" alt="Third slide">
            </div>
          </div>
                  ...
                  <div class="form-row">
                    <div class="form-group col-md-20">
                      ...
                      <script type="text/javascript">
                          ...
                          $pdo = new PDO('mysql:host=db5000232553.hosting-data.io;dbname=dbs227125', 'dbu387591', 'brun#ITPM8');
                          // Statement
                          $sql = "SELECT * FROM belegt where id_1 IS NOT NULL and id_2 IS NOT NULL and id_3 IS NOT NULL and id_4 IS NOT NULL";
...
<div class="container">
  <p>
    Einfaches Handling an Deck kombiniert mit guten Segeleigenschaften und einem hohen Grad an Vielseitigkeit – Die
    Sun Odyssey zeigt bei Wind und Wellen sehr lebhafte Segeleigenschaften mit hohem Spaßfaktor. Mit offenem Deck
    und einer Badeplattform ist sie sowohl sportlich als auch familientauglich. Der negative Decksprung verleiht dem
    Boot eine optische Besonderheit. Zusätzliche Stauräume sind im Heck vorhanden. Die dichten Lukendeckel sorgen
    für gute Ventilation in den Achterkabine.
  </p>
  <p>
    Unter Deck befinden sich drei Kabinen. Eine Duschkabine mit großzügigen Platzverhältnissen. In der mittleren
    Kabine befindet sich ein wuchtiger Salontisch für entspannte Abende mit 4 bis 6 Personen. Daneben eine
    funktionelle Pantry mit reichlich Stauraum. Die doppelflügelige Tür der Vorderkabine sorgt für ein angenehmes
    Raumgefühl mit ausreichend Platz für die Füße.
  </p>
