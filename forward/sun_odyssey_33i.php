...
$sql = "SELECT * FROM boots_typen where boots_typ_id = 4";
...
if (isset($_POST['buchungsenden'])) {
  ...
  $sqlPreisProWocheHS = "SELECT preis_pro_woche_hs FROM boots_typen WHERE boots_typ_id = 4";
  $sqlPreisProTagHS = "SELECT preis_pro_tag_hs FROM boots_typen WHERE boots_typ_id = 4";
  $sqlPreisProWocheNS = "SELECT preis_pro_woche_ns FROM boots_typen WHERE boots_typ_id = 4";
  $sqlPreisProTagNS = "SELECT preis_pro_tag_ns FROM boots_typen WHERE boots_typ_id = 4";
  $sqlKaution = "SELECT kaution FROM boots_typen WHERE boots_typ_id = 4";
  ...
  if (
    ...
    $schranke = false;
    $db = mysqli_connect('db5000232553.hosting-data.io', 'dbu387591', 'brun#ITPM8', 'dbs227125');
    $sqlAbfrageBelegt = "SELECT id_13 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
    foreach ($db->query($sqlAbfrageBelegt) as $row) {
      if ($row['id_13'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
        $boots_id = 14;
        break;
      } else {
        $boots_id = 13;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_14 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_14'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = 15;
          break;
        } else {
          $boots_id = 14;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_15 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_15'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = 16;
          break;
        } else {
          $boots_id = 15;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_16 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_16'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = '';
          break;
        } else {
          $boots_id = 16;
    ...
    switch ($boots_id) {
      case 13:
        $tabelle_boots_id = "id_13";
        break;
      case 14:
        $tabelle_boots_id = "id_14";
        break;
      case 15:
        $tabelle_boots_id = "id_15";
        break;
      case 16:
        $tabelle_boots_id = "id_16";
        break;
    }
        ...
        <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
          ...
          <div class="carousel-inner">
            <div class="carousel-item active">
              <img class="d-block w-100" src="../img/yachten/sun_odyssey_33i_1.png" alt="First slide">
            </div>
            <div class="carousel-item">
              <img class="d-block w-100" src="../img/yachten/sun_odyssey_33i_2.png" alt="Second slide">
            </div>
            <div class="carousel-item">
              <img class="d-block w-100" src="../img/yachten/sun_odyssey_33i_3.png" alt="Third slide">
            </div>
          </div>
                  ...
                  <div class="form-row">
                    <div class="form-group col-md-20">
                      ...
                      <script type="text/javascript">
                          ...
                          $pdo = new PDO('mysql:host=db5000232553.hosting-data.io;dbname=dbs227125', 'dbu387591', 'brun#ITPM8');
                          // Statement
                          $sql = "SELECT * FROM belegt where id_13 IS NOT NULL and id_14 IS NOT NULL and id_15 IS NOT NULL and id_16 IS NOT NULL";
...
<div class="container">
  <p>
    Besonders eindrucksvoll: das Wohnraumangebot und die Segelleistungen der Sun Odyssey 33i, deren Geschicke Sie bequem im Cockpit sitzend intuitiv und ganz entspannt mit Hilfe des leichtgängigen Ruders und der stets in greifbarer Nähe befindlichen Bedienelemente lenken. 
    Wenn das keine schönen Aussichten sind! 
    Vergessen Sie den Alltag und genießen Sie Ihr Tourenseglerglück. 
    Unter Deck bestimmt erstklassige Verarbeitung das Bild. 
    Sie werden begeistert sein von der großen, funktionellen Pantry, dem Kartentisch, wo sämtliche Bordfunktionen zusammenlaufen, der komfortablen Kajüte, der großzügigen, komplett ausgestatteten Nasszelle und den Kabinen, die viel Komfort, jede Menge Stauraum und schöne Aussichten durch zahlreiche Rumpffenster bieten.
  </p>
</div>
