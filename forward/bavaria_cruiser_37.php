...
$sql = "SELECT * FROM boots_typen where boots_typ_id = 2";
...
if (isset($_POST['buchungsenden'])) {
  ...
  $sqlPreisProWocheHS = "SELECT preis_pro_woche_hs FROM boots_typen WHERE boots_typ_id = 2";
  $sqlPreisProTagHS = "SELECT preis_pro_tag_hs FROM boots_typen WHERE boots_typ_id = 2";
  $sqlPreisProWocheNS = "SELECT preis_pro_woche_ns FROM boots_typen WHERE boots_typ_id = 2";
  $sqlPreisProTagNS = "SELECT preis_pro_tag_ns FROM boots_typen WHERE boots_typ_id = 2";
  $sqlKaution = "SELECT kaution FROM boots_typen WHERE boots_typ_id = 2";
  ...
  if (
    ...
    $schranke = false;
    $db = mysqli_connect('db5000232553.hosting-data.io', 'dbu387591', 'brun#ITPM8', 'dbs227125');
    $sqlAbfrageBelegt = "SELECT id_5 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
    foreach ($db->query($sqlAbfrageBelegt) as $row) {
      if ($row['id_5'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
        $boots_id = 6;
        break;
      } else {
        $boots_id = 5;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_6 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_6'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = 7;
          break;
        } else {
          $boots_id = 6;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_7 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_7'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = 8;
          break;
        } else {
          $boots_id = 7;
    ...
    if ($schranke == false) {
      $sqlAbfrageBelegt = "SELECT id_8 FROM belegt WHERE datum between '$startDatumDB' and '$endDatumDB'";
      foreach ($db->query($sqlAbfrageBelegt) as $row) {
        if ($row['id_8'] != null) { //Wenn Boot buchbar ist --> Buchungsformular.php
          $boots_id = '';
          break;
        } else {
          $boots_id = 8;
    ...
    switch ($boots_id) {
      case 5:
        $tabelle_boots_id = "id_5";
        break;
      case 6:
        $tabelle_boots_id = "id_6";
        break;
      case 7:
        $tabelle_boots_id = "id_7";
        break;
      case 8:
        $tabelle_boots_id = "id_8";
        break;
    }
        ...
        <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
          ...
          <div class="carousel-inner">
            <div class="carousel-item active">
              <img class="d-block w-100" src="../img/yachten/bavaria_cruiser_37_1.png" alt="First slide">
            </div>
            <div class="carousel-item">
              <img class="d-block w-100" src="../img/yachten/bavaria_cruiser_37_2.png" alt="Second slide">
            </div>
            <div class="carousel-item">
              <img class="d-block w-100" src="../img/yachten/bavaria_cruiser_37_3.png" alt="Third slide">
            </div>
          </div>
                  ...
                  <div class="form-row">
                    <div class="form-group col-md-20">
                      ...
                      <script type="text/javascript">
                          ...
                          $pdo = new PDO('mysql:host=db5000232553.hosting-data.io;dbname=dbs227125', 'dbu387591', 'brun#ITPM8');
                          // Statement
                          $sql = "SELECT * FROM belegt where id_5 IS NOT NULL and id_6 IS NOT NULL and id_7 IS NOT NULL and id_8 IS NOT NULL";
...
<div class="container">
  <p>
    Die Cruiser 37 ist eine Fahrtenyacht für Segler, die es gern sportlich mögen, das Freizeitleben aber nicht vernachlässigen möchten. 
    Das Ganze mit einem äußerst komfortablen Raumangebot – Die Bavaria Cruiser 37 bietet Sportlichkeit und Genuss par excellence und überzeugt nicht nur durch gute, sondern auch durch sichere Segeleigenschaften.
  </p>
  <p>
    Unter Deck wirkt es viel größer, als man es von einer Segelyacht in dieser Größe erwartet. 
    Der große Salon bietet Platz für 6 Personen. 
    Und was das Raumgefühl betrifft: in etwa wie auf einer 40‐Fuß‐Yacht. 
    Erleben Sie es selbst. 
    Design mit Liebe zum Detail auch in der Pantry Küche. 
    In der Vorderkabine befindet sich ein Doppelbett mit Platz zum Räkeln ohne anzuecken. 
    Und am nächsten Morgen duschen? 
    In allen Badezimmern finden sich entsprechende Einrichtungen, damit der Tag gut startet. 
    Auch in der Achterkabine ist die Stehhöhe extra hoch, sodass kein Crewmitglied Abstriche machen muss.
  </p>
